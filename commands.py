...
class sublimeDir(Command):
    def execute(self):
        self.fm.execute_command(
            f'sublime_text . -n &> /dev/null & disown')
...
class sublimeSingle(Command):
    def execute(self):
        if self.fm.thisdir.marked_items:
            ...
            self.fm.execute_command(
                f'''sublime_text {' '.join(f"'{w}'" for w in sel_list)} -n''')
        else:
            target_filename = self.fm.thisfile.path
        if not os.path.exists(target_filename):
            self.fm.notify("The given file does not exist!", bad=True)
            return
        self.fm.execute_command(
            f"sublime_text '{target_filename}' -n")
...
class ln(Command):
    def execute(self):
        ...
        self.fm.execute_command(
            f"ln -s {self.fm.thisfile} /home/user/Desktop/paths")
...
class runbuild(Command):
    ...
    def runfile(self, ext, file):
        if ext == 'py':
            self.fm.execute_command(
                f"path/to/builds/runpy.sh {file} &> /dev/null & disown")
        if ext == 'js':
            self.fm.execute_command(
                f"path/to/builds/runjs.sh {file} &> /dev/null & disown")
        if ext == 'go':
            self.fm.execute_command(
                f"path/to/builds/rungo.sh {file} &> /dev/null & disown")
...
class ranger_here(Command):
    def execute(self):
        self.fm.execute_command(
            f"xfce4-terminal --hold -x ranger . & /dev/null & disown")
...
class shutter_edit(Command):
    def execute(self):
        ...
        self.fm.execute_command(
            f"""xfce4-terminal -e '''bash -c "shutter {file}"'''""")
